...
const AddDTR = ({selectedUserName,LanguageOpt}) => {
  console.log('Selected User Name ' + JSON.stringify(selectedUserName));
  const [isLoading, setIsLoading] = useState(false);
  const {width, height} = Dimensions.get('window');
  let mount_type_info = ['Plinth (Cube)', 'Column (T-Shaped) ', 'Double Pole','Single Pole'];
  let transformer_type_info = ['Step-Up', 'Step-Down'];
  let fencing_type_info = ['Barbed Mesh', 'Wire Mesh'];
  ...
  const [mount_type, setSelectedMountType] = useState();
  const [fence_type, setFenceType] = useState();
  const [transformer_type, setTransformerType] = useState();
  //  DTR_ID,TRANSFORMER_SNO, CAPACITY,MFID,RATIO, UTILITY_REFNO ,STRUCTURE_CODE
  ...
  const saveFieldDeviceDataToServer = async () => {
    ...
    try {
      ...
      var payloadTest = {
        ...
        structureCode: deviceDetails.STRUCTURE_CODE,
        dtrserialno: deviceDetails.DTR_ID,
        serviceNo: '',
        meterSerialNo: '',
        newMeterSerialNo: '',
        initialReading: 0,
        ...
        phaseType: '',
        assetType: 'DTR',
        capacity: parseInt(deviceDetails.CAPACITY),
        manufactureName: '',
        mountType: mount_type,
        utilityRefNo: deviceDetails.UTILITY_REFNO,
        tmusno: '',
        tmutype: '',
          ...
          return (
            <View style={{flexDirection: 'column'}}>
              ...
              <ScrollView style={{marginTop: 10, marginBottom: 400}}>
                ...
                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('DTR_ID')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'DTR_ID')}
                />
                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('TRANSFORMER_SNO')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'TRANSFORMER_SNO')}
                />

                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('CAPACITY')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'CAPACITY')}
                />
                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('MFID')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'MFID')}
                />
                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('RATIO')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'RATIO')}
                />

                <SelectDropdown
                  data={transformer_type_info}
                  onSelect={(selectedItem, index) => {
                    setTransformerType(selectedItem);
                  }}
                  defaultButtonText={LanguageOpt('TRANSFORMER_TYPE')}
                  ...
                  buttonTextStyle={
                    (style = {textAlign: 'left', color: '#909090'})
                  }
                  renderDropdownIcon={() => {
                    // You can customize the dropdown icon here
                ...
                <SelectDropdown
                  data={mount_type_info}
                  onSelect={(selectedItem, index) => {
                    setSelectedMountType(selectedItem);
                  }}
                  defaultButtonText={LanguageOpt('MOUNT_TYPE')}
                  buttonTextAfterSelection={(selectedItem, index) => {
                    return selectedItem;
                  }}
                  rowTextForSelection={(item, index) => {
                ...
                <SelectDropdown
                  data={fencing_type_info}
                  onSelect={(selectedItem, index) => {
                    setFenceType(selectedItem);
                  }}
                  defaultButtonText={LanguageOpt('FENCE_TYPE')}
                  buttonTextAfterSelection={(selectedItem, index) => {
                    return selectedItem;
                  }}
                  rowTextForSelection={(item, index) => {
                ...
                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('UTILITY_REFNO')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'UTILITY_REFNO')}
                />
                <TextInput
                  style={styles.textInput}
                  placeholder={LanguageOpt('STRUCTURE_CODE')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'STRUCTURE_CODE')}
                />
