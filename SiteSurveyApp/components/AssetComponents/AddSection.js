...
import ConvertGeodesicAddress from '../ReUsableMethods/LocationOptions';
import DeviceInfo from 'react-native-device-info';
import { getUniqueId, getManufacturer } from 'react-native-device-info';
import TODOService from '../ReUsableMethods/TODOService';
import { Constants } from '../Constants/Constants';
//import * as Location from 'expo-location';
const AddSection = ({ selectedUserName, LanguageOpt }) => {
  ...
  const getDeviceIMEI = () => {
    ...
    console.log('IMEI ID UNIQUE ID ###### ' + JSON.stringify(deviceID));
  };

  // console.log(
  //   'Section Details',
  //   'Section details' + JSON.stringify(sectionDetails),
  // );
  ...
  const getLocationData = async (latitude, longitude) => {
    ...
    try {
      if (latitude && longitude) {
        var location = await addressService.convertGeodesicAddress(
          latitude,
          longitude,
        );
  ...
  const getGeoLocation = (latitude, longitude) => {
    ...
    // });
    Geocoder.init(Constants.GEOCODINGKEY);
  ...
  const saveSectionDataToServer = async () => {
    setIsLoading(true);
    try {
      ...
      var payloadTest = {
        ...
        inchargeUserName: '',
        inchargeName: sectionDetails.INCHARGE_NAME,
        contactNumber: parseInt(sectionDetails.INCHARGE_CONTACT_NUMBER),
        reasons: 'string',
      ...
      //console.log('SECTION Payload ' + JSON.stringify(response));
      if (response) {
        setIsLoading(false);
      }
    } catch (error) {
        ...
        if (isLoading) {
          ...
          return (
            <View style={{ flexDirection: 'column' }}>
              ...
              <View style={{ alignSelf: 'center' }}>
                <Text
                  ...
                  {' '}
                  {LanguageOpt('Add_Section_Details')}{' '}
                </Text>
              </View>
              ...
              <ScrollView style={{ marginTop: 10, marginBottom: 450 }}>
                <TextInput
                  ...
                  placeholder={LanguageOpt('SECTION_ID')}
                  placeholderTextColor="grey"
                  onChangeText={text => handleOnchange(text, 'SECTION_ID')}
                />
                <TextInput
                  style={styles.textInput}
            ...
            /> */}
                ...
                <TouchableOpacity
                  style={styles.button}
                  onPress={() => saveSectionDataToServer()}>
                  <Text
                    style={{
                      color: 'white',
                      fontSize: 18,
