...
const MeterDetailsServiceIntegration = ({ navigation, route }) => {
    ...
    // console.log('MAP VIEW PAGE', 'METERDETAILSSERVICEINTEGRATION', item);
    useEffect(() => {
        if (item !== null) {
            setAssetInstallationData(item);
            setAssetID(item.serviceNo);
            fetchData();
        }
    }, [item]);
...
export default MeterDetailsServiceIntegration;
