import React, {useState, useEffect} from 'react';
import {StyleSheet, View, Image} from 'react-native';
import {PieChart} from 'react-native-svg-charts';
import {G, Svg, Circle, Text as TextSvg} from 'react-native-svg';
import MyText from '../../ConsumerMobileSystemApp/SupportClasses/MyText';
import {getPieChartDataSteps} from './PieChartDataSteps';
const CIS_PieChart = ({listData}) => {
  const [height, setHeight] = useState(400);
  const [width, setWidth] = useState(340);
  const [SUM, setSUM] = useState(0);
  const pieChartDataSteps = getPieChartDataSteps(listData);
  const consumptionDataList = [];
  let sumOfConsumptions = 0;
  const data13 = [
    {
      key: 1,
      value: 50,
      text: 'Tuesday',
      svg: {fill: '#6D79C5'},
      arc: {cornerRadius: 10},
    },
    {
      key: 2,
      value: 50,
      text: 'Monday',
      svg: {fill: '#E27D39'},
      arc: {cornerRadius: 5},
    },
    {
      key: 3,
      value: 60,
      text: 'wednessday',
      svg: {fill: '#519F87'},
      arc: {cornerRadius: 5},
    },
    {
      key: 4,
      value: 35,
      text: 'Sunday',
      svg: {fill: '#668BBF'},
      arc: {cornerRadius: 10},
    },
    {
      key: 5,
      value: 50,
      text: 'Saturday',
      svg: {fill: '#D4972C'},
      arc: {cornerRadius: 10},
    },
    {
      key: 6,
      value: 35,
      text: 'Friday',
      svg: {fill: '#DA6C8C'},
      arc: {cornerRadius: 10},
    },
    {
      key: 7,
      value: 35,
      text: 'Thursday',
      svg: {fill: '#D46A6B'},
      arc: {cornerRadius: 10},
    },
    {
      key: 8,
      value: 35,
      text: 'Thursday',
      svg: {fill: '#D46A6B'},
      arc: {cornerRadius: 10},
    },
    {
      key: 9,
      value: 35,
      text: 'Thursday',
      svg: {fill: '#D46A6B'},
      arc: {cornerRadius: 10},
    },
    {
      key: 10,
      value: 70,
      text: 'Thursday',
      svg: {fill: '#D46A6B'},
      arc: {cornerRadius: 10},
    },
    {
      key: 11,
      value: 65,
      text: 'Thursday',
      svg: {fill: '#D46A6B'},
      arc: {cornerRadius: 10},
    },
    {
      key: 12,
      value: 45,
      text: 'Thursday',
      svg: {fill: '#D46A6B'},
      arc: {cornerRadius: 10},
    },
  ];

  const datamain = [
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 26.88,
      sts: '2024-02-02T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 120.96,
      sts: '2024-02-03T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 228.48,
      sts: '2024-02-04T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 141.12,
      sts: '2024-02-05T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 127.68,
      sts: '2024-02-06T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 13.46,
      sts: '2024-01-31T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 13.46,
      sts: '2024-02-01T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 13.46,
      sts: '2024-02-03T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 13.46,
      sts: '2024-02-05T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 87.62,
      sts: '2024-01-31T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 33.7,
      sts: '2024-02-01T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 33.7,
      sts: '2024-02-02T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 13.48,
      sts: '2024-02-03T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 80.88,
      sts: '2024-02-04T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 67.4,
      sts: '2024-02-05T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 40.44,
      sts: '2024-02-06T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 209.25,
      sts: '2024-01-31T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 108,
      sts: '2024-02-01T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 108,
      sts: '2024-02-02T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 162,
      sts: '2024-02-03T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 20.25,
      sts: '2024-02-04T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 148.5,
      sts: '2024-02-05T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 47,
      consumption: 60.75,
      sts: '2024-02-06T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-01-31T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-01T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-02T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-03T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-05T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-06T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-07T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-08T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1173,
      consumption: 0,
      sts: '2024-02-09T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1178,
      consumption: 0,
      sts: '2024-01-31T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1178,
      consumption: 0,
      sts: '2024-02-01T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1178,
      consumption: 0,
      sts: '2024-02-02T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1178,
      consumption: 0,
      sts: '2024-02-03T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1178,
      consumption: 0,
      sts: '2024-02-05T00:00:00',
    },
    {
      blockEnergyWhImport: null,
      meterId: 1234,
      consumption: 0,
      sts: '2024-01-31T00:00:00',
    },
  ];
 console.log("CIS PIE CHART ","CIS PIE CHART ########" + pieChartDataSteps.length , JSON.stringify(pieChartDataSteps ));

 useEffect(() => {   
    {pieChartDataSteps.map((pieItem) => {
        console.log(`ITEM: ${JSON.stringify(pieItem.value)}`),
            consumptionDataList.push(pieItem.value);        
    })}     
  
      // pieChartDataSteps.map(item =>(
      //       console.log(`ITEM: ${JSON.stringify(item.consumption)}`),
      //      consumptionDataList.push(item.consumption)
      // ))
  }, [pieChartDataSteps])
  console.log("Array Items Consumption ############# " + consumptionDataList.length, consumptionDataList);

  useEffect(() => {   
      for (let i = 0; i < consumptionDataList.length; i++ ) {
        sumOfConsumptions += consumptionDataList[i];
      }
      setSUM(sumOfConsumptions)
  }, [consumptionDataList])
  
  console.log(" SUM OF CONSUMPTIONS ##### "+ SUM)

  

  // const handleButtonClick = () => {
  //   if (subject != null) {
  //     RaiseComplaintToServer('RaiseGeneralComplaints');
  //     setTimeout(function () {
  //       console.log('Alert Shows After 5 Seconds of Delay.');
  //       // const response = await response.json();
  //       // const json =
  //     }, 100);
  //   }
  // };

  // const data = [];
  // console.log("PIE CHART DATA" + JSON.stringify(pieChartDataSteps));
  const randomColor = () =>
    ('#' + ((Math.random() * 0xffffff) << 0).toString(16) + '000000').slice(
      0,
      7,
    );

  // const pieData = data13
  //   .filter(value => value > 0)
  //   .map((value, index) => ({
  //     value,
  //     svg: {
  //       fill: randomColor(),
  //       onPress: () => console.log('press', index),
  //     },
  //     key: `pie-${index}`,
  //   }));

  // const Labels = ({slices, height, width}) => {
  //   return slices.map((slice, index) => {
  //     const {labelCentroid, pieCentroid, data13} = slice;
  //     return (
  //       <Text
  //         key={index}
  //         x={pieCentroid[0]}
  //         y={pieCentroid[1]}
  //         // fill={'black'}
  //         //textAnchor={'middle'}
  //         //alignmentBaseline={'middle'}
  //         fontSize={20}
  //         stroke={'black'}
  //         strokeWidth={0.2}>
  //         {data13.value}
  //       </Text>
  //     );
  //   });
  // };
  const Labels1 = ({slices, height, width}) => {
    return slices.map((slice, index) => {
      const {labelCentroid, pieCentroid, data13} = slice;
      return (
        <TextSvg
          key={index}
          x={pieCentroid[0]}
          y={labelCentroid[1]}
          fill={'white'}
          textAnchor={'middle'}
          alignmentBaseline={'middle'}
          fontSize={15}
          //stroke={'black'}
          // strokeWidth={0.2}
        >
          {data13.value}
        </TextSvg>
      );
    });
  };

  return (
    <View>
      <PieChart
        style={{
          height: height,
          width: width,
          alignSelf: 'center',
          borderColor: '#000',
          arc: {cornerRadius: 5},
        }}
        data={pieChartDataSteps}
        outerRadius="100%"
        innerRadius="35%"
        startAngle
        endAngle="-90"
        animate={{duration: 3000}}
        animationDuration={5000}
        //svg={{ stroke: 'rgb(134, 65, 244)' }}
      >
        {/* <Labels1/> */}
      </PieChart>

     
      <View
        style={{
          marginTop: 120,
          borderRadius: 80,
          borderColor: '#fff',
          position: 'absolute',
          alignSelf: 'center',
        }}>
        <MyText sum={SUM}/>
      </View>
    </View>
  );
};

export default CIS_PieChart;
/*

*/
/*
  <Image
        style={{
          width: 130,
          height: 150,
          marginTop: 130,   
          marginRight:40,       
          position: 'absolute',
          alignSelf:"center"
        }}
        source={{
          uri: 'data:image/png;base64,',
        }}>

      </Image>
*/
/*
 <Text
        style={{
          marginTop: 150,
          marginRight: 40,
          width: 100,
          height: 100,
          position: 'absolute',
          alignSelf: 'center',
        }}>
        HAI
      </Text>


       <Svg height="50%" width="50%" viewBox="0 0 100 100" >
        <Circle cx="50" cy="50" r="50" stroke="purple" strokeWidth=".5" fill="violet" />
      </Svg>

*/
